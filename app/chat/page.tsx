      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
            ...
            <div key={msg.id} className={`flex items-start gap-2 md:gap-3 ${msg.isMe ? 'flex-row-reverse' : ''}`}>
              ...
              <div className={`flex flex-col ${msg.isMe ? 'items-end' : 'items-start'} max-w-[80%]`}>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] md:text-xs text-gray-500">{msg.time}</span>
                  {msg.isMe && <CheckCheck className="w-3 h-3 md:w-4 md:h-4 text-[#9BB1D9]" />}
                </div>
                <div className={`px-3 md:px-4 py-2 md:py-3 rounded-2xl ${msg.isMe ? 'bg-[#C0F6CC] text-gray-900 rounded-l-lg' : 'bg-[#B3B3B3] rounded-r-lg'
                  } shadow-sm`}>
        ...
        <div className="bg-white border-t border-gray-200 px-4 md:px-6 py-3 md:py-4">
          <div className="flex items-center gap-2 md:gap-3">
            <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors"><Paperclip className="w-5 h-5 text-gray-600" /></button>
            <input
              type="text"
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="اكتب رسالتك هنا..."
              className="flex-1 px-4 py-2 md:py-3 bg-[#CADFC5]/50 border border-gray-200 rounded-xl outline-none text-sm md:text-base"
            />
